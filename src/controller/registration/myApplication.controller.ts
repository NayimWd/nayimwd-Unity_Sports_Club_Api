...
export const myApplication = asyncHandler(async(req, res)=>{
  ...
  if(!tournament){
    ...
    // find my application
    const application = await Registration.findOne({tournamentId: tournamentId, managerId: teamManager._id.toString()}).lean();

    if(!application){
        throw new ApiError(404, "You have not regestered for this tournament")
    };
