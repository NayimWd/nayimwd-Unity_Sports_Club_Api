...
export const addPlayingSquad = asyncHandler(async(req: Request, res: Response)=>{
  ...
  if (!manager?._id || manager.role !== "manager") {
    throw new ApiError(403, "Unauthorized request. Please log in as a manager.");
  }
  ...
  if (!tournamentId || !matchId || !teamId) {
    throw new ApiError(400, "Invalid request. Provide tournamentId, matchId, and teamId.");
  }
  if (!Array.isArray(addPlayers) || !Array.isArray(removePlayers)) {
    throw new ApiError(400, "'addPlayers' and 'removePlayers' must be arrays.");
  }
  ...
  if (match.status !== "scheduled") {
    throw new ApiError(400, "Playing squad cannot be updated after the match has started.");
  }
  ...
  // Remove players
  updatedPlayers = updatedPlayers.filter((playerId) => !removePlayers.includes(playerId));
  ...
  if (updatedPlayers.length !== 11) {
    throw new ApiError(400, `Playing squad must contain exactly 11 players. Current count: ${updatedPlayers.length}.`);
  }

  // Check if all players exist in the team
  const teamPlayers = await TeamPlayer.find({ playerId: { $in: updatedPlayers }, teamId });
  ...
  for (const teamPlayer of teamPlayers) {
    const playerRole = await PlayerProfile.findOne({ userId: teamPlayer.playerId.toString() }).select("player_role");
  ...
  // Role validations
  if (batsmen < 4) throw new ApiError(400, "Squad must contain at least 4 batsmen.");
  if (wkBatsman < 1) throw new ApiError(400, "Squad must contain at least 1 wicketkeeper-batsman.");
  if (allRounder < 1) throw new ApiError(400, "Squad must contain at least 1 all-rounder.");
  if (bowlers < 3) throw new ApiError(400, "Squad must contain at least 3 bowlers.");
  if (captainCount !== 1) throw new ApiError(400, "Squad must contain exactly 1 captain.");
  ...
  return res.status(200).json(new ApiResponse(200, playingSquad, "Playing squad updated successfully."));
});