...
export const getPlayingSquad = asyncHandler(async (req, res) => {
  ...
  if (!tournamentId || !matchId || !teamId) {
    throw new ApiError(
      400,
      "Invalid request. Please provide tournamentId, matchId, and teamId."
    );
  }
  ...
  const response = {
    ...
    matchId,
    teamId,
    captain: playingSquad
      ? playerDetails.find(
          (player) =>
            player._id.toString() === playingSquad.captain._id.toString()
        ) || null
      : null,
  ...
  return res
    ...
    .json(
      new ApiResponse(
        ...
        response,
        playingSquad
          ? "Playing squad retrieved successfully"
          : "No squad found, showing all team players"
      )
    );
});
